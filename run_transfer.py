...
def save_results_to_csv(args, mae, medae, r2):
    save_dir = "exps_results"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    file_path = os.path.join(save_dir, f"{args.target}.csv")
    ...
    with open(file_path, mode='a', newline='', encoding='utf-8-sig') as f:
        ...
        if not file_exist:
            writer.writerow(['Dataset', 'Fold', 'MAE', 'MedAE', 'R2'])

        writer.writerow([args.target, args.cvid, mae, medae, r2])
    ...
    if args.cvid == 9:
        ...
        with open(file_path, mode='r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                ...
                try:
                    maes.append(float(row[2]))
                    medaes.append(float(row[3]))
                    r2s.append(float(row[4]))
                except ValueError:
                    continue

        current_maes = maes[-10:]
        current_medaes = medaes[-10:]
        current_r2s = r2s[-10:]

        avg_mae = np.mean(current_maes)
        avg_medae = np.mean(current_medaes)
        avg_r2 = np.mean(current_r2s)
        
        
        std_mae = np.std(current_maes)
        std_medae = np.std(current_medaes)
        std_r2 = np.std(current_r2s)

        with open(file_path, mode='a', newline='', encoding='utf-8-sig') as f:
            writer = csv.writer(f)
            writer.writerow([])

            writer.writerow([args.target, 'Average', 
                             f"{avg_mae:.4f} ± {std_mae:.4f}", 
                             f"{avg_medae:.4f} ± {std_medae:.4f}", 
                             f"{avg_r2:.4f} ± {std_r2:.4f}"])
            print(f"Saved Average results to {file_path}")

save_results_to_csv(args, tst_mae, tst_medae, tst_r2)